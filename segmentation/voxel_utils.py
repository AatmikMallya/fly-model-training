...
def get_closest_true_idx(vals, idx):
    '''
    Given some index of vals, find the closest true index
    '''
...
def get_subvol_any_size(init_box_zyx, datatype):
    ...
    if np.any( np.array(init_box_zyx[1]) >= max_point_zyx ):
        ...
        append_zyx = np.max([append_zyx, npf.zeros(3, dtype=int)],axis=0)
...
def get_subvols_batched(init_boxes_zyx, datatype):
    kvstore_map = {
    ...
    def process_subvolume(subvol, context):
        ...
        # Pre-calculate final shape
        final_shape = list(subvol.shape)
        for i_axis in range(3):
            final_shape[i_axis] += prepend_zyx[i_axis] + append_zyx[i_axis]
        
        # Pre-allocate output array
        result = np.zeros(final_shape, dtype=subvol.dtype)
        
        # Calculate slices for efficient assignment
        src_slices = tuple(slice(None) for _ in range(3))
        dst_slices = tuple(slice(prepend_zyx[i], prepend_zyx[i] + subvol.shape[i]) 
                          for i in range(3))
        
        # Transpose and assign in one operation
        result[dst_slices] = np.transpose(subvol, axes=[2, 1, 0])
        
        # Update box coordinates
        box_zyx[0] -= prepend_zyx
        box_zyx[1] += append_zyx
        
        return result

    # Pre-process all boxes at once
    boxes = np.array([b for b in init_boxes_zyx]).astype(int)
    min_boxes = boxes[:, 0]
    max_boxes = boxes[:, 1]
    
    # Vectorized boundary calculations
    prepend_zyx = np.maximum(min_point_zyx - min_boxes, 0)
    append_zyx = np.maximum(max_boxes - max_point_zyx, 0)
    
    min_boxes_clipped = np.maximum(min_boxes, min_point_zyx)
    max_boxes_clipped = np.minimum(max_boxes, max_point_zyx)

    with ts.Batch() as batch:
        futures_and_contexts = []
        for i, (min_box, max_box, prepend, append) in enumerate(zip(
            min_boxes_clipped, max_boxes_clipped, prepend_zyx, append_zyx)):
            
            future = dataset_3d[min_box[2]:max_box[2], 
                              min_box[1]:max_box[1], 
                              min_box[0]:max_box[0]].read(batch=batch)
            
            context = {
                'init_box_zyx': boxes[i],
                'box_zyx': boxes[i].copy(),
                'prepend_zyx': prepend,
                'append_zyx': append
            }
            futures_and_contexts.append((future, context))

    # Process results and ensure return as proper numpy array
    results = [process_subvolume(future.result(), context) for future, context in futures_and_contexts]
...
def calc_plane_coords(theta, phi, box_zyx, height=None, width=None):
    ...
    x, y, z = calc_orthonormal_basis( [theta,phi] )
    if height is None:
...
def find_cross_section( theta, phi, subvol, height = None, width = None, skel_CS = None):
    '''
    Given a plane normal vector as (theta,phi), output the cross section of subvol.

    Inputs:
        theta : theta component of plane normal vector in radians
        phi : phi component of plane normal vector in radians
        subvol : 3D volume obtained from tensorstore
        height : height of cross_section you want outputted. If None, the function finds a reasonable choice
        width : width of cross_section you want outputted. If None, the function finds a reasonable choice
        skel_CS : boolean array of skeleton cross-section. Only keeps cross_section values in the skeleton cross section
    '''
...
def find_best_cross_section( subvol ):
    '''
    Given a 3D, boolean subvolume, find the cross_section that minimizes the cross_sectional area.
    There is a penalty included for when the cross-section is cropped by the bounds of subvol

        Inputs:
            subvol : boolean 3D subvol array
    '''
...
def get_border_mask(bool_array, border_type):
    '''
    Find the border pixels of plane_vals. If you want the border as the true pixels
    touching the false pixels, set border_type to 'on'. If you want the border as
    the false pixels touching the true pixels, set border_type to 'off'.

    This function works for 2D and 3D arrays

    Inputs:
        bool_array : boolean numpy array specifying which pixels belong to the foreground
        border_type : set to 'on' or 'off' depending on the type of border you want
    Outputs:
        border_mask : boolean array of same size as plane_vals
    '''
...
def is_cropped(vals, box_shape = None, theta=None, phi=None):
    '''
    Figure out if the cross_section obtained by theta, phi is cropped

    Inputs:
        vals : 2D or 3D boolean array of the foreground
        box_shape : shape of original 3D voume. Only required is vals is 2D
        theta : theta component of plane normal vector in radians. Only need this if theta is not None
        phi : phi component of plane normal vector in radians. Only need this if theta is not None
    Output:
        is_cropped : whether plane_vals is cropped
    '''
...
def get_full_mito_coords(mito_series):
    '''
    Get the full list of coordinates for this mitochondrion
    '''
...
def get_foreground_stats(vals, box_zyx, fill_holes = True, theta = None, phi = None, return_COM = True):
    '''
    Given a 3D or 2D boolean array of the foreground, compute statistics of the foregound

    Inputs:
        vals : boolean array of the foreground. Can be a 3D or 2D
        box_zyx : bounding box used to get the 3D values. Using theta, phi on that
                  3D volume would return plane_vals
        fill_holes : whether or not to fill holes in the foreground. This is usually yes
        theta : theta component of plane normal vector in radians. Only need this if vals is 2D and return_COM is true
        phi : phi component of plane normal vector in radians. Only need this if vals is 2D and return_COM is true
        return_COM : whether or not to return center of mass to the user
    Outputs:
        enclosed_space: If vals is 2D, this is the cross-sectional area.
                        If vals is 3D, this is the Volume.
        SA : If vals is 2D, this is the circumference.
             If vals is 3D, this is the surface area.
    '''
    if not np.any(vals):
        if not return_COM:
            return 0, 0
...
def get_CrossSection_info(bodyId, coord, dr, return_mito = False, return_grayscale = False, return_plane_coords = False, max_dr = 700):
    '''
    Given the bodyId of the neuron you're interested in, get the cross-section at
    the coordinate specified by coord
    Inputs:
        bodyId : bodyId of the neuron to analyze
        coord : coordinate (x,y,z) to center the analysis
        dr : initial guess of radius for cross-section
    '''
