...
def denoise_image(image, mask, weight=0.1):
    masked_image = apply_mask(image, mask)
    denoised = restoration.denoise_tv_chambolle(masked_image, weight=weight, channel_axis=None)
    return apply_mask(denoised, mask)

def get_slice_from_box(init_box_zyx, bodyId):
