...
def initialize_model(model_type="unet"):
       ...
       "vnet": {
        ...
        "dynunet": {
            ...
            "params": {
                ...
                "in_channels": 1,
                "out_channels": 1,
                # Configure kernel sizes for each level
                "kernel_size": [
                    (3, 3, 3),  # First level
                    (3, 3, 3),  # Second level
                    (3, 3, 3),  # Third level
        ...
        "basicunetplusplus": {  # UNet++ architecture
            ...
            "params": {
                ...
                "norm": ("instance", {"affine": True}),
                "dropout": 0.2,
                "upsample": "pixelshuffle",  # Change from deconv to nontrainable
                "deep_supervision": True
            }
        }
    }
    ...
    #     nn.Conv3d(1, 1, kernel_size=2, stride=1, padding=0)  # Add 1x1x1 conv
    # )
    ...
    # new_final = nn.Sequential(
    #     final_layer,  # Keep the ConvTranspose3d
    #     nn.Conv3d(1, 4, kernel_size=2, stride=1, padding='same'),
    #     nn.ReLU(),
    #     nn.Conv3d(4, 1, kernel_size=2, stride=1, padding='same')
    # )
    # model.model[-1] = new_final
...
# def bce_dice_loss(outputs, labels, pos_weight):
#     # Handle deep supervision outputs (list of tensors)
...
#             dice = 1 - (2. * intersection + eps) / (union + eps)
#             losses.append(dice)
#         return sum(losses) / len(losses)  # Average loss across all supervision outputs
...
# def bce_dice_loss(outputs, labels, pos_weight, alpha=0.5):
#     # if isinstance(outputs, list):
#     #     losses = []
#     #     for output in outputs:
#     #         bce = F.binary_cross_entropy_with_logits(output, labels)
...
#     #         losses.append(alpha * bce + (1-alpha) * dice)
...
def bce_dice_loss(outputs, targets, pos_weight, alpha=0.5, gamma=1):
    """
    Compute Focal Loss for 3D segmentation.
    
    Args:
        outputs: Raw logits from model (N, 1, D, H, W)
        targets: Binary ground truth (N, 1, D, H, W)
        alpha: Weighting factor for rare class (microtubules)
        gamma: Focusing parameter - higher values increase focus on hard examples
    """
    # # BCE loss with built-in sigmoid - more stable than separate ops
    # bce_loss = F.binary_cross_entropy_with_logits(outputs, targets, reduction='none')
    
    # # Get probabilities without separate sigmoid call
    ...
    # # Calculate modulating factor with epsilon
    ...
    return 0.05*dice + F.binary_cross_entropy_with_logits(
        outputs, 
        targets,
        # pos_weight=pos_weight * torch.ones_like(outputs),
        reduction='mean'
    )
...
def compute_segmentation_metrics(outputs, labels, threshold=0.5):
    """
    Compute metrics at two thresholds: standard (0.5) and strict (0.999)
    """
    # Handle deep supervision outputs - use the final (deepest) output
    if isinstance(outputs, list):
        outputs = outputs[-1]  # Take the last/deepest output
    ...
    for thresh, suffix in [(threshold, ''), (0.6, '_strict')]:
        ...
        # Store all metrics with appropriate suffix
        metrics.update({
        ...
        if suffix == '':  # Only for standard threshold
            metrics['f1'] = dice.item()  # F1 score is same as Dice for binary case
            ...
            if true_negatives + false_negatives > 0:  # Avoid division by zero
