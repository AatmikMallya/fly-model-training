    ...
    for key, _ in model_dict.items():
        if all([i not in key for i in skip_strings_in_pretrained]):
            ...
            assert model_dict[key].shape == pretrained_dict[key].shape, \
                f"The shape of the parameters of key {key} is not the same. Pretrained model: {pretrained_dict[key].shape}; your network: {model_dict[key].shape}. The pretrained model does not seem to be compatible with your network."
